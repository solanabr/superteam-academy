...
import { BookOpen, Clock, Zap, Users, ArrowRight } from "lucide-react";
...
export function CourseCard({ course, variant = "default" }: CourseCardProps) {
	...
	return (
		...
		>
			<div className={`relative ${isCompact ? "h-28" : "h-36"} bg-gradient-to-br ${gradient} overflow-hidden`}>
				...
				{course.enrolled && course.progress !== undefined && (
					<div className="absolute bottom-0 left-0 right-0 px-3 pb-2">
						...
						<span className="text-[10px] text-white/80 mt-0.5 block">{course.progress}%</span>
					</div>
				)}
				<div className="absolute bottom-3 left-3">
					<span className={`inline-block px-2 py-0.5 rounded-md text-xs font-medium backdrop-blur-sm border ${levelClass}`}>
			...
			<div className="flex-1 p-4 space-y-2">
				<h3 className={`font-semibold leading-tight line-clamp-2 group-hover:text-primary transition-colors ${isCompact ? "text-sm" : "text-base"}`}>
					{course.title}
				</h3>
