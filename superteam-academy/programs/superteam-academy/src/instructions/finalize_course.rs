...
pub fn handler(ctx: Context<FinalizeCourse>) -> Result<()> {
    ...
    // Mint completion bonus XP to learner
    let bonus_xp = course.completion_bonus_xp;
    if bonus_xp > 0 {
        utils::mint_xp(
...
pub struct FinalizeCourse<'info> {
    ...
    /// CHECK: Token-2022 ATA for creator's XP. Validated by Token-2022 program during mint CPI.
    #[account(mut)]
