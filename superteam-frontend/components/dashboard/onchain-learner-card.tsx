...
export function OnChainLearnerCard({ identity, onInitialized }: Props) {
    ...
    return (
      <div className="mb-6 rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4">
        <div className="flex items-center gap-2 text-emerald-400">
          ...
          <p className="text-sm font-medium">On-chain learner profile is active.</p>
  ...
  async function handleInitialize() {
    ...
    if (publicKey.toBase58() !== identity.profile.walletAddress) {
      setStatusError("Connected wallet does not match the authenticated wallet.")
      return
    }
    ...
    setStatusError(null)
    try {
      const signature = await sendInitLearner(sendTransaction, identity.profile.walletAddress)
      setStatusMessage(`Learner profile initialized. Tx: ${signature.slice(0, 8)}...`)
      onInitialized?.()
      window.setTimeout(() => {
      ...
      const message =
        error instanceof Error ? error.message : "Failed to initialize on-chain learner."
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-foreground">Initialize on-chain learner profile</p>
            <Badge variant="outline" className="border-primary/40 text-primary">
              {identity.chain.cluster}
            </Badge>
          </div>
          <p className="mt-1 text-xs text-muted-foreground">
            Required once per wallet to store XP, level, and streak directly on-chain.
          </p>
        </div>
        <Button onClick={handleInitialize} disabled={isSubmitting} className="gap-2">
