...
| **C5's pinned stack does not resolve** | CAT-12 **RESOLVED (2026-07-25, #598)**              | @solana/pay@1.0.23 peers kit ^6.9.0 (known). **@solana/subscriptions@0.4.0 — the mandatory C5 dependency the spec orders pinned exactly — peers `@solana/kit: ^6.4.0`, which also excludes 7.0.0.** There is no newer release. Token peers are disjoint too (pay ^0.12.0 vs @x402/svm ^0.9.0; latest is 0.15.0).                                                                                                                                                                                                                                                                                                                                          | **RESOLVED → §3 item 9 (#598): pin the whole set to the kit-6 shelf — `@solana/kit@6.10.0` + `@solana-program/token@0.14.0`, no override.** Two facts the spec missed: token `latest` 0.15.0 peers kit `^7.0.0` (so token must also drop to 0.14.0), and `@x402/svm`→`compute-budget@0.11.0` peers kit `^5.0` (a strict-peer transitive seam). Intersection `[6.5.0, 7.0.0)`, non-empty. See §3 item 9 for the full ruling and decay clock. |
...
**9. Resolve C5's dependency seam.** — #598 · **RESOLVED (2026-07-25)**

> **RESOLVED (2026-07-25, #598) — pin C5's whole install set to the kit-6 shelf: `@solana/kit@6.10.0` + `@solana-program/token@0.14.0`, keeping `@solana/subscriptions@0.4.0` and `@x402/{core,svm,express,fetch}@2.19.0`. No peer override.** This is ruling **(a)** — a specific kit 6.x — because the constraint intersection over the packages C5 _actually installs_ is non-empty. It is **not** ruling (b) (override) and **not** (c) (a newer subscriptions); both are rejected below.
>
> **Registry re-verified live 2026-07-25 (`npm view … peerDependencies`). C5's actual install set and their `@solana/kit` peer ranges:**
>
> | Package (what C5 installs)                   | Version          | `@solana/kit` peer | Interval          |
> | -------------------------------------------- | ---------------- | ------------------ | ----------------- |
> | `@solana/subscriptions`                      | `0.4.0` (latest) | `^6.4.0`           | `[6.4.0, 7.0.0)`  |
> | `@solana-program/token`                      | `0.14.0`         | `^6.5.0`           | `[6.5.0, 7.0.0)`  |
> | `@x402/svm`                                  | `2.19.0`         | `>=5.1.0`          | `[5.1.0, ∞)`      |
> | `@x402/core`, `@x402/fetch`, `@x402/express` | `2.19.0`         | _(no kit peer)_    | _(unconstrained)_ |
>
> **Constraint intersection** = `[6.5.0, 7.0.0) ∩ [6.4.0, 7.0.0) ∩ [5.1.0, ∞)` = **`[6.5.0, 7.0.0)`** → highest available kit in-window = **`6.10.0`** (verified latest 6.x). Non-empty ⇒ ruling (a) resolves with **zero overrides**; a scratch `npm install --dry-run` of the six packages above with `@solana/kit@6.10.0` succeeds (93 pkgs, exit 0).
>
> **Two facts the source spec missed — both change the ruling, and are the signal the gate needs:**
> 1. **`@solana-program/token@0.15.0` (the current `latest`) peers `@solana/kit ^7.0.0`, not ^6.** The spec noted "token latest is 0.15.0" (CAT-14) but never checked its kit peer. So the naive "pin C5 to kit 6.x" while keeping token at `latest` does **NOT** resolve: `[6.4.0,7.0.0) ∩ [7.0.0,8.0.0) = ∅`. The whole set must move to the kit-6 shelf **together** — hence token is pinned back to **`0.14.0`** (the highest token version whose kit peer, `^6.5.0`, still admits 6.x). This is the load-bearing correction.
> 2. **A second, transitive seam that neither the spec nor #598 names:** `@x402/svm@2.19.0` depends on `@solana-program/compute-budget@^0.11.0`, and `compute-budget@0.11.0` peers `@solana/kit ^5.0` — which excludes **both** kit 6 and kit 7. It is **orthogonal** to the subscriptions seam (present on every kit path) and **harmless under a normal install** (npm nests a kit-5.5.1 subtree for compute-budget while the root stays 6.10.0), but it makes `npm install --strict-peer-deps` (and pnpm's default strict-peer mode) fail. **Install guidance for C5's package and for the in-lesson repro: a plain `npm install` / `pnpm install` resolves; do not pass `--strict-peer-deps`.** This transitive under-declaration is itself teachable seam material.
>
> **Why (b) and (c) lose:**
> - **(c) is not available:** `@solana/subscriptions` `latest` is `0.4.0` (`^6.4.0`); its only newer dist-tag `beta` is `0.4.0-rc.2` — _older_ than 0.4.0 and also `^6.4.0`. No release admits kit 7.
> - **(b) override is rejected as unsafe here:** forcing kit 7.0.0 hard-fails `npm install` with `ERESOLVE` (`peer @solana/kit@"^6.4.0" from @solana/subscriptions@0.4.0` vs root `7.0.0`) — a **genuine** conflict, not a strict-mode artifact — and installs only under `--legacy-peer-deps`/`--force`, which npm itself warns is "an incorrect (and potentially broken) dependency resolution." `subscriptions@0.4.0` was never built or tested against kit 7; silencing its peer in a **mandatory** dependency that learners run against live devnet risks a runtime break the pure-fixture sandbox cannot catch. An override is the wrong thing to bake into the course's canonical lockfile when a clean, override-free pin exists.
>
> **In-lesson (this is a feature, per the spec's own thesis, not an embarrassment to hide):** C5 pins **kit 6.10.0 while C1–C4 pin 7.0.0** — a deliberate _regression_ that L1/L3 state out loud. The mandatory `@solana/subscriptions` client (pre-1.0, for an audited mainnet program) has not shipped a kit-7 build, so C5 pins the newest kit its **whole** install set agrees on. Learners read the peer ranges and see _why_ the pin is what it is — the pre-1.0 ecosystem reality the whole catalog is themed around, made concrete.
>
> **Decay clock — revisit when any fires; the version-stamp frontmatter + CI currency check (#597 / item 8) is the catch mechanism (this is exactly why item 8 is "load-bearing, not hygiene"):**
> 1. `@solana/subscriptions` publishes a release whose `@solana/kit` peer admits 7.x (or 8.x) → move C5 up to realign with C1–C4 and bump token to `0.15.0+`.
> 2. `@solana/kit` `8.0.0` stabilizes to `latest` (8.0.0-canary publishes daily; today's is `8.0.0-canary-20260724151329`) → C1–C4's kit-7 pins revisit and C5's kit-6 pin is two majors behind.
> 3. `@x402/svm` widens its `@solana-program/compute-budget` range off `0.11.x` (kit `^5.0`) → the `--strict-peer-deps` transitive seam clears.

_Original (2026-07-25, superseded by the resolution above; preserved for audit trail):_
