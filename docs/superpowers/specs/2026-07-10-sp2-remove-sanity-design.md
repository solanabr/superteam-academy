...
2. **Images live in `courses-academy` git.** The projector's asset pipeline (`lib/content-sync/assets.ts`) serves images from `cdn.sanity.io`, and thumbnails were Studio-authorable (git-unsourced). **Measured 2026-07-11 against prod Sanity (4e3i2wwc/production) + the content repo: there are NO images to migrate** — zero course thumbnails, zero instructor avatars, zero in-lesson images in live content (the dataset's only 2 image assets are orphaned test uploads referenced by nothing; the repo's only image reference is in `_template`). So the fix is forward-looking, not a migration: add `thumbnail` to the content schema (done — app PR #416) and have the SP2-A compiler copy `assets/**` into `apps/web/public/content-assets/` and rewrite markdown/thumbnail paths, so images added later version with content. Zero external image host; `cdn.sanity.io` exits config at SP2-D with nothing orphaned.
   **Instructor avatars are NOT repo content (owner decision 2026-07-11):** they come from the platform profile — the instructor's wallet-linked account (`profiles.avatar_url`), set by the instructor themselves. The read layer resolves instructor display identity as `instructor.wallet → profiles` (same wallet key SP1's teach viewer uses); the content-schema `avatar` field is ignored by the compiler and slated for removal from the schema.
...
| **Images** (thumbnails + in-lesson markdown assets — NOT avatars)                                                                                                                                                                    | files in `courses-academy` git → compiler copies into `apps/web/public/content-assets/` + rewrites paths. Instructor avatars come from the platform profile (`profiles.avatar_url` keyed by `instructor.wallet`), not the repo. `cdn.sanity.io` exits `next.config.mjs` remotePatterns/CSP at SP2-D                                                                                                                                                                                                                                                                                   |
...
Prereqs: app PR #416 (schema `thumbnail`, merged first — courses-academy CI validates against this repo's main) + a one-line courses-academy PR vendoring the updated `schema/course.schema.json` (no thumbnails exist to commit; they arrive later as ordinary content PRs). Then: `compile-content.ts`, `content.lock`, the committed bundle + assets, the typed `server-only` `lib/content/*` API — NO consumer switches. CI freshness check + canonicalized parity anchor green = proof the two sources are equivalent before anything flips.
...
- Any change to what content EXISTS (SP2 changes where it's read from, not the content itself; there are no images to relocate — see decision 2).
- Mainnet.
