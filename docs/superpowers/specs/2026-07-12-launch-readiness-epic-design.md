...
4. **#376 Next.js highs:** dismiss-with-reason now (assess reachability — App Router + next-intl, so the Pages-router i18n bypass likely N/A), transitive dev-only criticals via overrides; Next 15 is a separate post-launch migration.
...
- **Supply-chain** (one PR, `.github/` + deps): **#376** (transitive crit/highs via `pnpm.overrides`; the 4 `next` highs dismissed-with-reason per decision 4), **#382** (SHA-pin all Actions + dependabot github-actions), **#381** (content-lint symlink — emit generic parse error, drop `err.message`), **#437** (X-Frame-Options `DENY` + `frame-ancestors 'none'`).
