...
**Vs Blueshift:** they own deep program-side education (Pinocchio, assembly, anchor-init challenge builds). Our lane is the **wrapper**: the only _sequenced, graded, version-stamped, credentialed_ path from web2 JS to first deployed program to first paid work, terminating at a Superteam Earn submission, plus the two lanes Blueshift has no course for at all (Rust-the-language, x402/agentic payments). We do not compete on program internals; we compete on the on-ramp, the credential+terminus, and the Earn pipeline. _(Corrected per #608: "Brazil-first / zero-setup" as the differentiator is retired — Blueshift ships 206/206 pt-BR pages and a "comece do zero" path, and Solana Playground has offered zero-setup browser build+deploy for years; see UNIFIED-LAUNCH-SPEC §1 "Positioning vs Blueshift".)
...
| Pedagogy #7 / UIUX R8 — cohort leaderboard                           | **MISSING**                                            | get_leaderboard is global-absolute (CORRECTED: DEFAULT p_limit=20, caller-supplied — not intrinsically top-100); no cohort tables (20 tables verified, none — corrected per #645: `challenge_assists` is the 20th); no user-centered rank query; platform has **no scheduler of any kind** (no vercel cron, pg_cron deliberately avoided).                                                                                                                                                                                                                                                        |
...
**Confusable-concept interleaved sets (pedagogy roadmap #12):** review sessions may mix items across confusable concepts (PDA vs ATA, Anchor constraint types) — interleaving lives ONLY inside review sets; never interleave lesson prose. _(Corrected per #608: the g≈0.34 / g=−0.39 figures are struck — g≈0.34 is math-task interleaving and g=−0.39 is word-list interleaving; neither measures prose. The surviving prose citation is expository text g=0.21, p=.119 (n.s., not harmful). The "review-sets only, never reorder prose" ruling stands on the null result, not on a harm number.)_
...
3. **Leaderboard prizes, XP→token bridges, monetary XP salience** (soulbound illiquidity is the moat). _(Corrected per #608: the d=−0.88 attribution is struck — that figure is performance-contingent-less-than-max reward vs a no-feedback control in Deci/Koestner/Ryan, not a measurement of leaderboards or monetary salience. The no-prize / no-bridge decision survives on PED-10 + PED-14 + UIU-09 and the airdrop-extraction literature, not on −0.88.)_
