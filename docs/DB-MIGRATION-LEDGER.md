...
| `20260731120000_reminder_consent.sql`                                                         | `20260731120914`                | —                                                                                                                                                                        | #869 — reminder consent columns on `email_subscriptions` + `email_reminder_log` ledger + 4 RPCs. **MUST be MCP-applied before the PR merges** (#731: Vercel auto-deploys from `main`, so the settings toggle and the cron route would hit missing objects). Record the MCP-stamped version here on apply.                                                                                                                                                                                      |
| `20260731150000_kind_scoped_unsubscribe.sql`                                                  | **PENDING-GATE-VERIFY**         | —                                                                                                                                                                        | #896 — kind-scoped unsubscribe: adds `email_subscriptions.reminder_unsubscribe_token` (+ UNIQUE + `chk_email_subscriptions_distinct_tokens`), re-scopes `unsubscribe_reminders_by_token` / `unsubscribe_by_token` to their own token column and makes `claim_due_session_reminders` hand out the reminder token. Depends only on `email_subscriptions` + `reminder_consent` (both applied). **MUST be MCP-applied before the PR merges** (#731). Record the MCP-stamped version here on apply. |
