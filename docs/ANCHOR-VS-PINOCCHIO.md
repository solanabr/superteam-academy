...
| Artifact selection | `target/deploy/onchain_academy.so`                                                                                                                                                                                                                                                                                                                                        | `scripts/select-program.sh {anchor\|pinocchio}` swaps the active slot (genesis for `anchor test`, CU harness) and records `.active-program`; **`anchor build` silently restores Anchor bytes** — always re-select afterwards |
...
| Fuzzing            | nightly `Fuzz (litesvm)` workflow — `onchain-academy/tests/fuzz` drives the pinocchio `.so` on litesvm (Trident was dropped in 09-2026: its embedded solana-sbpf 0.11 cannot execute a platform-tools v1.54 binary)                                                                                                                                                                                                                                                                    |                                                                                                                                                                                                                                       |
