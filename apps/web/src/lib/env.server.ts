...
// z.default() only activates on `undefined`, not "". A set-but-blank env var
// passes the type check but fails min(1). Preprocess "" → undefined so blank
// placeholder values are treated as unset rather than as invalid input.
 ...
 * treat "" as unset (optStr/optUrl) so a blank Vercel placeholder doesn't fail
