...
export function getTerm(idOrAlias: string, locale: string = 'en'): GlossaryTerm | undefined {
  const match = allTerms.find(
    (t) =>
      t.id === idOrAlias || t.aliases?.some((a) => a.toLowerCase() === idOrAlias.toLowerCase())
  );
...
export function searchTerms(query: string, locale: string = 'en'): GlossaryTerm[] {
...
export function getTermsByCategory(category: string, locale: string = 'en'): GlossaryTerm[] {
  return allTerms
