...
| `lesson_completed`             | `{ lessonId, courseId, signature?, source: "live" \| "replay" }`                                                | `lessons/[id]/lesson-client.tsx` on completion API success (`live`) and `lib/lessons/replay-banked.ts` per replayed banked completion (`replay`, no signature). A test-out pass also writes `user_progress` rows in bulk and fires only `test_out_graded` — the credited lesson ids are not knowable client-side, so those completions are still absent here                                                                    |
