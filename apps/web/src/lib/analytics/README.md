...
| `attempt_gate_overridden`      | `{ lessonId, challengeKind, courseId? }`                                                                        | `trackAttemptGateOverridden()` — learner took the free one-tap override on the attempt-gate nudge; never a penalty, purely a content-quality signal (high override rate ⇒ likely starter-code problem); session-deduped per lesson (#865)                                                                                                                                                                                       |
