...
export function useConexo(seedVal: string = new Date().toDateString()) {
  ...
  useEffect(() => {
    ...
    chosenCategories.forEach((cat, index) => {
      const allTermsInCat = getTermsByCategory(cat, 'pt-BR').filter(t => t.term.length <= 25); // avoid super long terms
  ...
  const submitGuess = useCallback(() => {
    ...
    const firstCat = boardTiles.find(t => t.id === selectedIds[0])?.categoryId;
    ...
    if (isCorrect && firstCat) {
      ...
      if (matchedGroup) {
        ...
        setBoardTiles(prev => prev.filter(t => !selectedIds.includes(t.id)));
        setSelectedIds([]);
  ...
  }, [selectedIds, status, boardTiles, targetGroups, solvedGroups.length, mistakesRemaining]);
