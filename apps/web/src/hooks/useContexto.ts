...
const STOP_WORDS = new Set(['a', 'o', 'e', 'do', 'da', 'de', 'em', 'para', 'com', 'um', 'uma', 'os', 'as', 'is', 'the', 'of', 'in', 'and', 'to', 'for', 'on', 'with', 'by']);
...
function calculateSimilarity(t1: GlossaryTerm, t2: GlossaryTerm) {
  ...
  if (t1Def.includes(t2.term.toLowerCase()) || t2Def.includes(t1.term.toLowerCase())) {
    score += 100;
  }
...
export function useContexto(seedVal: string = new Date().toDateString()) {
  ...
  const [rankingMap, setRankingMap] = useState<Map<string, { rank: number, simScaled: number }>>(new Map());
  ...
  const submitGuess = useCallback(() => {
    ...
    const validMatch = allTerms.find(t => t.term.toLowerCase() === termToMatch || t.id.toLowerCase() === termToMatch);
    ...
    const rankInfo = rankingMap.get(validMatch.term.toLowerCase()) || rankingMap.get(validMatch.id.toLowerCase());
    if (rankInfo) {
