...
/**
 * "Sign in with GitHub", routed through Dynamic instead of Supabase OAuth.
 *
 * Same account either way — `/api/auth/dynamic` matches on the GitHub-verified
 * email — but this path also leaves the learner with an embedded Solana wallet,
 * which the Supabase OAuth path does not.
 *
 * The click ends in a full-page navigation to GitHub, so nothing after
 ...
 */
export function DynamicGithubSignIn({ disabled }: { disabled: boolean }) {
  ...
  const handleClick = async () => {
    setStarting(true);
    setError(null);
    trackEvent("auth_method_selected", { method: "dynamic_github" });
    try {
      ...
      // and calls `oauthVerify` (LINK this GitHub account to the existing
      ...
      console.error("[DynamicGithubSignIn] redirect failed:", err);
      setError(t("githubSignInFailed"));
      setStarting(false);
    }
  };
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        ...
        {starting ? t("connecting") : t("signInWithGitHub")}
      </Button>
      {error && (
