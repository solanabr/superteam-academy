...
interface ReviewStripProps {
  /** Authoritative due count — the session's own length (#977). */
  count: number;
  /** Due lesson titles, in session order. */
  titles: string[];
}
 ...
 * card: it names a few due items and deep-links into /review. Presentational
 * since #1096 — the server shell reads the SAME capped `buildReviewSession` as
 * the /review page (#977: count ≡ what the session serves) via
 * `loadReviewDue` and passes the result down; renders nothing when the queue
 * is empty — no empty-state chrome on the dashboard.
 */
export function ReviewStrip({ count, titles }: ReviewStripProps) {
