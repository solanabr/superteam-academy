...
| `/api/enroll/custodial`           | POST     | Required | Fully server-side enrolment for walletless (Google/GitHub-only) accounts via the derived custodial Academy Wallet (`CUSTODIAL_WALLET_SECRET`, unset = 503 `custodialDisabled`). Derives the keypair from the session's user id, binds it as `profiles.wallet_address` (prefs `walletProvider: "custodial"`), signs+confirms `enroll` server-side, mirrors the `enrollments` row. 409 `walletLinked` when a different wallet is already linked. |
