      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">{t("xpToLevel", { current: currentXP - currentLevelXP, needed: nextLevelXP - currentLevelXP, level: currentLevel + 1 })}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                {streakLoading ? (
                  ...
                  <p className="text-2xl font-bold text-streak-orange">{t("days", { count: streak?.current ?? 0 })}</p>
                )}
              </div>
            ...
            <p className="text-xs text-muted-foreground mt-1">{t("longest", { count: streak?.longest ?? 0 })}</p>
          </CardContent>
        </Card>
      ...
      <Card className="mb-8 min-h-[140px]">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg flex items-center gap-2">
            <Flame className="h-5 w-5 text-streak-orange" /> {t("streakCalendar")}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {streak ? (
            <>
              ...
              <div className="mt-3 flex gap-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm bg-solana-green" /> {t("active")}</span>
                <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm bg-blue-400" /> {t("frozen")}</span>
                <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm bg-muted" /> {t("inactive")}</span>
              </div>
            </>
          ) : (
            <div className="flex gap-1.5 flex-wrap">
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Button asChild size="sm" variant="ghost">
                          <Link href={`/courses/${course.slug}`}>
                            {tc("continue")} <ArrowRight className="h-3.5 w-3.5 ml-1" />
                          </Link>
                        </Button>
                      </div>
          ...
          {completedCourses.length > 0 && (
            <>
              <h2 className="text-xl font-semibold mt-8 mb-4 flex items-center gap-2">
                <Trophy className="h-5 w-5 text-xp-gold" /> {t("completedCourses")}
              </h2>
                      ...
                      <CardContent className="p-3 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{t("xpEarned", { amount: course.xpTotal })}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button asChild size="sm" variant="outline">
                            <Link href={`/certificates/${course.trackId}`}>{t("certificate")}</Link>
                          </Button>
                          <Badge className="bg-solana-green/10 text-solana-green border-0">{tc("completed")}</Badge>
                        </div>
                      </CardContent>
                    </Card>
                  );
