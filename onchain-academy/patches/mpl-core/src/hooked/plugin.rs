...
pub(crate) fn registry_records_to_plugin_list(
    ...
    // Box the accumulator to keep the large PluginsList off the stack frame,
    // avoiding the SBF 4096-byte stack offset limit.
    let result = registry_records
        .iter()
        .try_fold(
            Box::new(PluginsList::default()),
            |mut acc, record| -> Result<Box<PluginsList>, std::io::Error> {
                if PluginType::from_u8(record.plugin_type).is_some() {
                    ...
                    let plugin =
                        Plugin::deserialize(&mut &account_data[record.offset as usize..])?;

                    assign_plugin(&mut acc, plugin, base);
                }
                Ok(acc)
            },
        )?;
...
#[inline(never)]
fn assign_plugin(acc: &mut PluginsList, plugin: Plugin, base: BasePlugin) {
    match plugin {
