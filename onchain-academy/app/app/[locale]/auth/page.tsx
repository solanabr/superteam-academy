...
function SignupBonuses() {
    ...
    const bonuses = [
        { icon: "⚡", labelKey: "xpBonus", color: "text-neon-green" },
        { icon: "🏅", labelKey: "earlyAdopterBadge", color: "text-amber-400" },
        { icon: "❄️", labelKey: "streakFreezes", color: "text-neon-cyan" },
    ];

    return (
        <div className="space-y-2">
            <div className="text-[10px] text-zinc-600 uppercase tracking-widest font-bold font-mono">
                <span className="text-neon-green/40">// </span>{t("signupRewards")}
            </div>
            <div className="flex gap-2">
                {bonuses.map((b, i) => (
                    ...
                    >
                        ...
                        <span className={`text-[10px] font-bold ${b.color}`}>{t(b.labelKey)}</span>
                    </motion.div>
                ))}
            </div>
        </div>
...
function AuthButton({
    ...
    const config = {
        wallet: {
            ...
            desc: mode === "signup" ? t("connectToEarnXp") : t("signInWithWallet"),
        ...
        google: {
            ...
            label: "Google",
            desc: mode === "signup" ? t("quickStartLinkLater") : t("signInWithGoogle"),
            color: "text-blue-400",
        ...
        github: {
            ...
            label: "GitHub",
            desc: mode === "signup" ? t("syncDevProfile") : t("signInWithGithub"),
            color: "text-zinc-300",
            borderColor: "border-white/[0.06]",
...
function LinkedAccount({
    ...
    const config = {
        wallet: { icon: <Wallet className="w-4 h-4" />, label: t("solanaWallet"), color: "text-neon-green" },
        ...
        <div className={`flex items-center gap-3 p-3 border transition-all font-mono ${linked ? "border-neon-green/20 bg-neon-green/[0.03]" : "border-white/[0.06] bg-white/[0.01]"}`}>
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    ...
                    {required && !linked && (
                        <span className="text-[8px] font-black uppercase tracking-widest text-amber-400">{t("required")}</span>
                    )}
                </div>
                {linked && detail && (
            ...
            {linked ? (
                <div className="flex items-center gap-1 text-neon-green">
                    ...
                    <span className="text-[10px] font-bold">{t("linked")}</span>
                </div>
            ) : (
                <button
                    onClick={onLink}
        ...
        <div className="space-y-4 font-mono">
            ...
            {!walletLinked && (
                ...
                >
                    ...
                    <div>
                        <div className="text-[11px] font-bold text-amber-400">{t("walletRequired")}</div>
            ...
            {walletLinked && googleLinked && githubLinked && (
                ...
                >
                    ...
                    <span className="text-[11px] text-neon-green font-bold">{t("allAccountsLinked")}</span>
                </motion.div>
            )}
...
function SidebarStats() {
    ...
    const stats = [
        { icon: Zap, labelKey: "xpDistributed", value: "8.5M+", color: "text-neon-green" },
        { icon: Trophy, labelKey: "nftsEarned", value: "32K+", color: "text-amber-400" },
        { icon: Flame, labelKey: "activeStreaks", value: "4.2K", color: "text-orange-400" },
        { icon: Crown, labelKey: "avgLevel", value: "8.4", color: "text-neon-purple" },
    ];
    ...
    return (
        <div className="hidden lg:flex flex-col justify-center space-y-8 px-8 xl:px-12">
                ...
                {stats.map((stat, i) => (
                    ...
                    >
                        ...
                        <div className="flex-1">
                            <div className="text-[10px] text-zinc-600 uppercase tracking-wider font-bold">{t(stat.labelKey)}</div>
                        </div>
            ...
            >
                ...
                <div className="text-[10px] text-zinc-600 uppercase tracking-widest font-bold font-mono">
                    <span className="text-neon-green/40">$ </span>{t("playerNew")}
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <div className="font-mono">
                        <div className="text-sm font-bold text-white">{t("newPlayer")}</div>
                        <div className="text-[10px] text-zinc-500">{t("levelOne")} • 0 {t("xp")} • {t("noStreak")}</div>
                    </div>
                </div>
                <div className="h-1.5 bg-white/5 overflow-hidden">
                    <div className="h-full w-0 bg-neon-green" />
                </div>
                <div className="text-[10px] text-zinc-600 text-center font-mono">{t("signUpToActivate")}</div>
            </motion.div>
        </div>
    );
}
        ...
        if (connected && publicKey && selectedMethod === "wallet" && signMessage) {
            (async () => {
                try {
                    ...
                    setIsLoading(false);
                    setAuthError(err instanceof Error ? err.message : t("walletAuthFailed"));
                }
            })();
        }
    }, [connected, publicKey, selectedMethod, signMessage, mode, login, t]);
            ...
            if (idToken) {
                ...
                (async () => {
                    try {
                        ...
                        setIsLoading(false);
                        setAuthError(err instanceof Error ? err.message : t("googleAuthFailed"));
                    }
                })();
            }
        }
    }, [session, mode, login, t]);
    ...
    const handleAuth = (method: AuthMethod) => {
        ...
        if (method === "github") {
            ...
            if (GITHUB_CLIENT_ID) {
                ...
                setIsLoading(false);
                setAuthError(t("githubClientIdNotConfigured"));
            }
            return;
        }
    };
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center gap-3 mb-3">
                                <span className="text-neon-green font-mono text-sm">{">"}</span>
                                <span className="font-mono text-xs uppercase tracking-[0.3em] text-zinc-500">
                                    {mode === "signup" ? t("newPlayer") : t("returningPlayer")}
                                </span>
                                <div className="flex-1 h-px bg-white/[0.06]" />
                            </div>
                            <h1 className="text-2xl md:text-3xl font-black text-white font-mono">
                                {mode === "signup" ? t("startYourQuest") : t("welcomeBack")}
                            </h1>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex-1 h-px bg-white/[0.06]" />
                                <span className="text-[10px] text-zinc-600 uppercase tracking-widest font-bold font-mono">{t("orContinueWith")}</span>
                                <div className="flex-1 h-px bg-white/[0.06]" />
                            </div>
                        ...
                        {authError && (
                            ...
                            >
                                ...
                                <div>
                                    <div className="text-[11px] font-bold text-red-400">{t("authError")}</div>
                        ...
                        <p className="text-[10px] text-zinc-700 text-center leading-relaxed font-mono">
                            {t("byContinuing")}{" "}
                            <a href="#" className="text-zinc-500 underline hover:text-neon-green transition-colors">{t("termsOfService")}</a>
                            {" "}{t("and")}{" "}
                            <a href="#" className="text-zinc-500 underline hover:text-neon-green transition-colors">{t("privacyPolicy")}</a>.
                        </p>
                    </motion.div>
                </div>
            </div>
