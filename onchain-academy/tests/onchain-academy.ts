...
import { type Program, BN, AnchorError } from "@coral-xyz/anchor";
import type { SuperteamAcademy } from "../target/types/superteam_academy";
...
describe("onchain-academy", () => {
	...
	before(async () => {
		...
		[enrollmentPda, enrollmentBump] = PublicKey.findProgramAddressSync(
			[Buffer.from("enrollment"), Buffer.from(courseId), learner.publicKey.toBuffer()],
			program.programId
		);
		...
		for (const wallet of [learner.publicKey, creator.publicKey]) {
			const sig = await provider.connection.requestAirdrop(wallet, 5 * LAMPORTS_PER_SOL);
	...
	describe("1. Initialize", () => {
		it("initializes config and XP mint", async () => {
			...
			expect(configAccount.backendSigner.toBase58()).to.equal(authority.publicKey.toBase58());
			expect(configAccount.xpMint.toBase58()).to.equal(xpMintKeypair.publicKey.toBase58());
			expect(configAccount.bump).to.equal(configBump);

			// Verify auto-registered backend minter role
			const minterRole = await program.account.minterRole.fetch(backendMinterRole);
			expect(minterRole.minter.toBase58()).to.equal(authority.publicKey.toBase58());
	...
	describe("2. Update Config", () => {
		it("rotates backend signer", async () => {
			...
			const configAccount = await program.account.config.fetch(configPda);
			expect(configAccount.backendSigner.toBase58()).to.equal(newSigner.publicKey.toBase58());
			...
			expect(restored.backendSigner.toBase58()).to.equal(authority.publicKey.toBase58());
		});
		...
		it("no-op update with null keeps signer unchanged", async () => {
			...
			expect(after.backendSigner.toBase58()).to.equal(before.backendSigner.toBase58());
		});
	...
	describe("3. Create Course", () => {
		it("creates course with all fields", async () => {
			...
			expect(course.minCompletionsForReward).to.equal(MIN_COMPLETIONS_FOR_REWARD);
	...
	describe("8. Close Enrollment", () => {
		it("closes completed enrollment immediately", async () => {
			const balanceBefore = await provider.connection.getBalance(learner.publicKey);
			...
			const enrollmentInfo = await provider.connection.getAccountInfo(enrollmentPda);
			expect(enrollmentInfo).to.be.null;

			const balanceAfter = await provider.connection.getBalance(learner.publicKey);
		...
		it("close incomplete enrollment before 24h cooldown fails", async () => {
			...
			const [freshEnrollPda] = PublicKey.findProgramAddressSync(
				[Buffer.from("enrollment"), Buffer.from(freshId), learner.publicKey.toBuffer()],
				program.programId
			);
	...
	describe("9. Multi-learner flow", () => {
		...
		before(async () => {
			...
			[learner2EnrollPda] = PublicKey.findProgramAddressSync(
				[Buffer.from("enrollment"), Buffer.from(courseId), learner2.publicKey.toBuffer()],
				program.programId
			);
		...
		it("second learner enrolls, completes, and finalizes same course", async () => {
			...
			const enrollment = await program.account.enrollment.fetch(learner2EnrollPda);
			expect(enrollment.completedAt).to.not.be.null;
			...
			expect(Number(l2ata.amount)).to.equal(XP_PER_LESSON * LESSON_COUNT + EXPECTED_BONUS_XP);
	...
	describe("11. Creator reward threshold", () => {
		...
		before(async () => {
			for (const wallet of [threshLearner.publicKey, threshCreator.publicKey]) {
				const sig = await provider.connection.requestAirdrop(wallet, 3 * LAMPORTS_PER_SOL);
				await provider.connection.confirmTransaction(sig, "confirmed");
			}
	...
	describe("12. Prerequisite enforcement", () => {
		...
		it("enroll with completed prerequisite succeeds", async () => {
			...
			expect(enrollment.course.toBase58()).to.equal(advancedCoursePda.toBase58());
		});
	});
	...
	describe("Phase 6: Credentials", () => {
		...
		before(async () => {
			...
			const umiAuthority = umi.eddsa.createKeypairFromSecretKey(authority.payer.secretKey);
		...
		it("issues credential NFT for completed course", async () => {
			...
			const umi = createUmi(provider.connection.rpcEndpoint, { commitment: "confirmed" }).use(
				mplCore()
			);
			const asset = await fetchAssetV1(umi, fromWeb3JsPublicKey(credentialKeypair.publicKey));
	...
	describe("13. PDA seed validation (security)", () => {
		...
		before(async () => {
			...
			const tx = new anchor.web3.Transaction().add(createAtaAIx).add(createAtaBIx);
			await provider.sendAndConfirm(tx);
		});
	...
	describe("15. Minter Roles", () => {
		const _MPL_CORE_PROGRAM_ID = new PublicKey("CoREENxT6tW1HoK8ypY1SxRMZTcVPm7R94rH4PZNhX7d");
	...
	describe("16. Achievement System", () => {
		...
		it("create_achievement_type", async () => {
			...
			expect(achievement.creator.toBase58()).to.equal(authority.publicKey.toBase58());
		...
		it("award_achievement", async () => {
			...
			const receipt = await program.account.achievementReceipt.fetch(achievementReceiptPda);
			expect(receipt.asset.toBase58()).to.equal(assetKeypair.publicKey.toBase58());
			...
			const asset = await fetchAssetV1(umi, fromWeb3JsPublicKey(assetKeypair.publicKey));
		...
		it("achievement supply exhaustion (maxSupply=1)", async () => {
			...
			try {
				...
				const anchorErr = err as AnchorError;
				expect(anchorErr.error.errorCode.code).to.equal("AchievementSupplyExhausted");
			}
		});
	});
	...
	describe("19. update_config deactivates old MinterRole", () => {
		...
		it("backend_signer rotation deactivates old MinterRole via remaining_accounts", async () => {
			...
			expect(config.backendSigner.toBase58()).to.equal(newSigner.publicKey.toBase58());
