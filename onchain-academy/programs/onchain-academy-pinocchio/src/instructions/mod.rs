//! One module per instruction, mirroring the Anchor handlers 1:1. Each
//! `process` validates accounts in Anchor's exact field/constraint order
//! (so the first failing check — and its error code — match the oracle),
//! then runs the identical business logic.

pub mod award_achievement;
pub mod close_course;
pub mod close_enrollment;
pub mod complete_lesson;
pub mod create_achievement_type;
pub mod create_course;
pub mod deactivate_achievement_type;
pub mod enroll;
pub mod finalize_course;
pub mod initialize;
pub mod issue_credential;
pub mod register_minter;
pub mod revoke_minter;
pub mod reward_xp;
pub mod update_config;
pub mod update_course;
pub mod update_minter;
pub mod upgrade_credential;
