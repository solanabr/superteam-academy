...
pub struct AwardAchievement<'info> {
    ...
    #[account(mut, seeds = [b"achievement", achievement_type.achievement_id.as_bytes()], bump = achievement_type.bump)]
    ...
    #[account(
        ...
        seeds = [b"achievement_receipt", achievement_type.achievement_id.as_bytes(), recipient.key().as_ref()],
        bump,
    )]
    ...
    /// CHECK: Metaplex Core collection. Validated by constraint.
    #[account(mut, constraint = collection.key() == achievement_type.collection @ AcademyError::Unauthorized)]
    pub collection: AccountInfo<'info>,

    /// CHECK: Recipient of the achievement NFT.
    pub recipient: AccountInfo<'info>,

    /// CHECK: Recipient's Token-2022 ATA. Validated by CPI.
    #[account(mut)]
    pub recipient_token_account: AccountInfo<'info>,

    /// CHECK: XP mint. Validated by constraint.
    #[account(mut, constraint = xp_mint.key() == config.xp_mint @ AcademyError::Unauthorized)]
    ...
    #[account(address = crate::utils::TOKEN_2022_PROGRAM_ID)]
