...
pub struct FinalizeCourse<'info> {
    ...
    /// CHECK: Learner's Token-2022 ATA.
    #[account(mut)]
    pub learner_token_account: AccountInfo<'info>,

    /// CHECK: Creator's Token-2022 ATA.
    #[account(mut)]
    pub creator_token_account: AccountInfo<'info>,

    /// CHECK: Validated by constraint.
    #[account(constraint = creator.key() == course.creator @ AcademyError::Unauthorized)]
    ...
    #[account(constraint = backend_signer.key() == config.backend_signer @ AcademyError::Unauthorized)]
