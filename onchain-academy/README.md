...
On-chain program for the Superteam Academy learning platform — courses, soulbound XP tokens, Metaplex Core credentials, and achievements on Solana.

The program is Pinocchio-only:

| Crate                                | Framework        | Role                          |
| ------------------------------------ | ---------------- | ----------------------------- |
| `programs/onchain-academy-pinocchio` | Pinocchio 0.11.2 | CU-optimized production build |

See [SPEC.md](../docs/SPEC.md) for the full program specification and [ANCHOR-VS-PINOCCHIO.md](../docs/ANCHOR-VS-PINOCCHIO.md) for the historical comparison against the retired Anchor build and the measured CU comparison ([tests/CU_COMPARISON.md](./tests/CU_COMPARISON.md)).
...
**18 instructions · 6 PDA types · 34 error variants · 18 events**
...
| `close_course`                | Free a Course PDA (incl. stale pre-resize accounts) for recreation          |
| `register_minter`             | Register an external XP minter with per-call + lifetime caps                |
| `update_minter`               | Update a minter's per-call / lifetime XP caps                               |
...
```bash
# Build the program (cargo build-sbf --tools-version v1.54)
pnpm build:pinocchio

# Deploy artifact for the self-owned devnet instance (--features fresh-id;
# see docs/DEPLOY-PROGRAM.md § "Fresh devnet instance")
pnpm build:pinocchio:fresh

# Pinocchio byte-parity tests (layouts, discriminators, CPI wire bytes)
pnpm test:layout

# Single-VM integration suite (all 18 instructions, happy + error paths)
pnpm test:integration

# CU comparison (local only; writes tests/CU_COMPARISON.md)
pnpm cu:compare
