...
import { ArrowLeftIcon, ChatCircleTextIcon, PlusIcon, FunnelIcon } from '@phosphor-icons/react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[60px_1fr_350px] lg:grid-rows-[48px_1fr] min-h-screen lg:h-screen lg:overflow-hidden max-w-full relative z-10">
        ...
        <main className="p-4 lg:p-8 flex flex-col gap-10 overflow-visible lg:overflow-y-auto w-full">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-ink-secondary/20 dark:border-border pb-6">
            ...
            <Button asChild className="bg-ink-primary hover:bg-ink-primary/90 text-bg-base font-bold uppercase tracking-widest rounded-none flex items-center gap-2">
          ...
          <div className="flex flex-col gap-6">
             <div className="flex justify-between items-center mb-2 border-b border-border pb-4">
                 ...
                 <Button variant="ghost" className="h-8 rounded-none text-[10px] uppercase tracking-widest font-bold text-ink-secondary hover:bg-ink-primary/10 transition-colors flex items-center gap-1.5 px-3 border border-border">
             ...
             <div className="border border-ink-secondary/20 dark:border-border bg-surface/50 backdrop-blur-sm divide-y divide-ink-secondary/20 dark:divide-border">
               {mockDiscussions.map(disc => (
                 <Link href={`/community/${disc.slug}`} key={disc.id} className="block group relative bg-surface/50 hover:bg-ink-primary/5 transition-colors">
                   {/* Corner Accents */}
                   ...
                   <div className="p-4 flex justify-between gap-4">
                     <div>
                       ...
                       <h3 className="font-display text-2xl leading-none mt-1 group-hover:text-ink-primary transition-colors">{disc.title}</h3>
                     ...
                     <div className="text-right shrink-0 flex flex-col justify-between items-end">
                       ...
                       <div className="text-[10px] text-ink-tertiary mt-2 uppercase tracking-widest">{disc.lastActive}</div>
                     </div>
                   </div>
                 </Link>
               ))}
