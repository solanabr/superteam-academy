...
export function TypingAnimation({
	...
	const highlightSyntax = (code: string) => {
		...
		const tokenRegex =
			/("[^"]*")|\b(pub|fn|let|if|return|mut)\b|(&[a-zA-Z_]\w*)|\b([A-Z]\w*)\b|\b(process_instruction|next_account_info|msg!|Ok|Err)\b|(::|!=|\?)/g;

		return code.replace(
			tokenRegex,
			(match, string, keyword, ref, type, func, op) => {
				if (string)
					return `<span class="text-green-600 dark:text-[#f1fa8c]">${string}</span>`;
				if (keyword)
					return `<span class="text-purple-600 dark:text-[#ff79c6]">${keyword}</span>`;
				if (ref)
					return `<span class="text-blue-600 dark:text-[#50fa7b]">${ref}</span>`; // References - blue in light, green in dark
				if (type)
					return `<span class="text-teal-600 dark:text-[#8be9fd]">${type}</span>`;
				if (func)
					return `<span class="text-yellow-600 dark:text-[#f1fa8c]">${func}</span>`;
				if (op)
					return `<span class="text-pink-600 dark:text-[#ff79c6]">${op}</span>`;
				return match;
			},
		);
	};
			...
			<div
				...
				dangerouslySetInnerHTML={{
					__html:
						...
						(!isFocused
							? '<span class="inline-block w-[2px] h-[1em] bg-ink-primary ml-px animate-pulse align-middle"></span>'
							: "") +
						"<br/>",
				}}
			/>
