...
export function CodeChallenge({
  ...
  const [activeTab, setActiveTab] = useState<"tests" | "output" | "discussion">(
    "tests",
  );
  ...
  const handleRunCode = useCallback(async () => {
    ...
    try {
      ...
      const data: { results: TestCase[]; output: string; allPassed: boolean } =
        await res.json();
  ...
  const handleResetCode = useCallback(() => {
    ...
    setTestResults(
      challenge.testCases.map((tc) => ({ ...tc, passed: undefined })),
    );
    setRunOutput("");
  }, [challenge]);
...
function DesktopPromptPanel({
  ...
  lesson: Lesson;
  challenge: NonNullable<Lesson["challenge"]>;
  courseSlug: string;
    ...
    <div className="flex h-full flex-col bg-[#1e1e1e]">
      ...
      <div className="border-t border-[#333]">
        <div className="flex items-center gap-px bg-[#252526]">
          ...
          >
            {t("challenge.tests")}
            {testResults.some((r) => r.passed !== undefined) && (
        ...
        <div
          className={cn(
            "overflow-y-auto bg-[#1e1e1e]",
            activeTab === "discussion" ? "h-64 p-4" : "h-36 p-3",
          )}
        >
          {activeTab === "tests" ? (
            ...
            <OutputDisplay
              output={runOutput}
              placeholder={t("challenge.outputPlaceholder")}
            />
          ) : (
          ...
          <div className="text-xs text-[#888]">
            ...
            {completed && (
              <span className="flex items-center gap-1 text-brazil-green">
                ...
                {t("challenge.challengeCompleted")}
              </span>
            )}
          </div>
          {completed ? (
            ...
            >
              ...
              {t("challenge.markCompleteXP", { xp: lesson.xpReward })}
            </button>
          ) : (
            <button
              onClick={onRunCode}
