...
import { Copy, Check, ExternalLink, LogOut, ChevronDown, Wallet } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuContent,
...
function WalletIcon({ walletClientType, size = 16 }: { walletClientType: string; size?: number }) {
  const s = size;
  ...
  if (walletClientType === "coinbase_wallet") {
    return (
      <svg width={s} height={s} viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="64" y="72" textAnchor="middle" fontSize="28" fontWeight="bold" fill="#0052FF" fontFamily="sans-serif">C</text>
      </svg>
    );
  }
