...
export function LearningProgressProvider({
  ...
  const loadAll = useCallback(async () => {
    ...
    const [xpRes, streakRes, achievementsRes, allProgressRes] =
    ...
    const allProgress =
      allProgressRes.status === "fulfilled" ? allProgressRes.value : [];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setState((prev) => ({
        ...
        xp: xpRes.status === "fulfilled" ? xpRes.value : prev.xp,
        streak:
          streakRes.status === "fulfilled" ? streakRes.value : prev.streak,
        achievements:
          achievementsRes.status === "fulfilled"
            ? achievementsRes.value
            : prev.achievements,
  ...
  const enrollInCourse = useCallback(
    async (courseId: string, totalLessons: number, meta?: ActivityMeta) => {
      ...
      trackEvent({
        ...
        params: { course_slug: courseId, course_title: courseId },
      });
    ...
    async (achievementId: string) => {
      ...
      trackEvent({
        ...
        params: {
          ...
          achievement_name: `achievement_${achievementId}`,
        },
      });
      const achievementName =
        state.achievements.find((a) => a.id === achievementId)?.name ??
        `Achievement #${achievementId}`;
      recordActivity(state.userId, "achievement_earned", {
