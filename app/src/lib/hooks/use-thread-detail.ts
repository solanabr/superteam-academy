...
export function useThreadDetail(threadId: string) {
  ...
  const voteThread = useCallback(async (value: VoteValue) => {
    if (value === 0) return;
    setThread((prev) => {
      if (!prev) return prev;
      const oldVote = prev.userVote;
      const newVote: VoteValue = oldVote === value ? 0 : value;
      return { ...prev, userVote: newVote, voteScore: prev.voteScore + (newVote - oldVote) };
    });
    try {
  ...
  return { thread, isLoading, addComment, editComment, deleteComment, voteThread, voteComment, refresh: fetchThread };
}
