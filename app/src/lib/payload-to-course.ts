...
function lexicalToHtml(data: unknown): string | undefined {
  ...
  try {
    const html = convertLexicalToHTML({ data: data as Parameters<typeof convertLexicalToHTML>[0]["data"] });
...
export function payloadCourseToCourse(doc: PayloadCourse): Course {
  const modules = (doc.modules ?? []).map(
    (m: PayloadCourse, mIdx: number) => {
      const lessons = (m.lessons ?? []).map(
        (l: PayloadCourse, lIdx: number) => {
            ...
            challenge:
              isChallenge && l.challenge
                ? {
                    ...
                    prompt: lexicalToHtml(l.challenge.prompt) ?? l.challenge.prompt ?? "",
      ...
      const challengeCount = lessons.filter((l: { type: string }) => l.type === "challenge").length;
