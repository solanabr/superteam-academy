...
/**
 * Check if a specific lesson index is complete in the bitmap.
 * Enrollment tracks lessons as [u64; 4] = 256 bits.
 */
...
export function getLessonProgress(lessonFlags: BN[], lessonCount: number): number {
    if (lessonCount === 0) return 0;
    return Math.round((countCompletedLessons(lessonFlags) / lessonCount) * 100);
}
