        ...
        return (
            <div className="min-h-screen">
                <div className="flex flex-col items-center justify-center mt-32 gap-4 text-center px-4">
                    ...
                    <h2 className="font-heading text-2xl font-bold">{t("connect_wallet")}</h2>
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="glass rounded-2xl p-6 flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-xs text-[hsl(var(--muted-foreground))] font-semibold uppercase tracking-wide mb-1">{t("xp_balance")}</p>
                            <p className="font-heading font-bold text-2xl gradient-text">{xp.loading ? "..." : formatXP(xp.balance)}</p>
                            <p className="text-sm text-[hsl(var(--muted-foreground))]">{xp.levelTitle} · {t("level")} {xp.level}</p>
                        </div>
                    </div>
                    ...
                    <div className="glass rounded-2xl p-6 flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-xs text-[hsl(var(--muted-foreground))] font-semibold uppercase tracking-wide mb-1">{t("streak")}</p>
                            <p className="font-heading font-bold text-2xl">3 <span className="text-base font-normal text-[hsl(var(--muted-foreground))]">{t("days")}</span></p>
                            <p className="text-sm text-orange-400">{t("streak_desc")} 🔥</p>
                        </div>
                    </div>

                    {/* Wallet */}
                    <div className="glass rounded-2xl p-6">
                        <p className="text-xs text-[hsl(var(--muted-foreground))] font-semibold uppercase tracking-wide mb-3">{t("wallet")}</p>
                        ...
                        >
                            {t("view_explorer")} <ArrowRight className="w-3 h-3" />
                        </Link>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2">
                        <h2 className="font-heading text-xl font-semibold mb-4">{t("continue_learning")}</h2>
                            ...
                            {inProgressCourses.map((course) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-[hsl(var(--muted-foreground))] mt-1">2 / {course.lessonCount} {t("lessons")}</p>
                                    </div>
                    ...
                    {/* Stats sidebar */}
                    <div className="space-y-4">
                        <h2 className="font-heading text-xl font-semibold">{t("your_stats")}</h2>

                        {[
                            { label: t("stat_lessons"), value: "12", icon: BookOpen, color: "text-purple-400 bg-purple-400/10" },
                            { label: t("stat_achievements"), value: "3", icon: Award, color: "text-yellow-400 bg-yellow-400/10" },
                            { label: t("stat_rank"), value: "#42", icon: TrendingUp, color: "text-green-400 bg-green-400/10" },
