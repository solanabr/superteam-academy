...
import { finalizeCourseSchema } from "@/lib/api-schemas";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = finalizeCourseSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const { courseId, learner } = parsed.data;
    ...
    const creatorPubkey = courseData.creator;
    ...
    const [learnerAtaInfo, creatorAtaInfo] = await Promise.all([
      connection.getAccountInfo(learnerAta),
    ...
    console.error("finalize_course error:", message);
