import type { CloseEnrollmentBridgeRequest } from "@/lib/onchain/bridge-types";
import {
  badRequest,
...
export async function POST(request: Request) {
  const action = "close_enrollment" as const;
  const { ctx, error } = await requireAuthenticatedBridgeContext(action);
  if (error || !ctx) return error;

  const { payload, error: payloadError } =
    await parseBridgeJson<CloseEnrollmentBridgeRequest>(request, action);
