import type { UpgradeCredentialBridgeRequest } from "@/lib/onchain/bridge-types";
import { normalizeBackendBridgeError, submitUpgradeCredentialTx } from "@/lib/onchain/backend-tx";
import {
  badRequest,
  parseBridgeJson,
...
export async function POST(request: Request) {
  const action = "upgrade_credential" as const;
  const { ctx, error } = await requireAuthenticatedBridgeContext(action);
  if (error || !ctx) return error;

  const { payload, error: payloadError } =
    await parseBridgeJson<UpgradeCredentialBridgeRequest>(request, action);
  ...
  try {
    const tx = await submitUpgradeCredentialTx({
    ...
    return NextResponse.json({
      ...
      action,
      code: "TX_SUBMITTED",
      message: "upgrade_credential transaction submitted on-chain.",
