...
export default function EditCoursePage() {
      ...
      try {
        ...
        if (res.ok && data.course) {
          setCourse({
            ...
            xpReward: data.course.xpReward || 500,
            track: data.course.track || '',
          ...
          if (data.course.modules?.length > 0) {
            setOpenModules([data.course.modules[0].id]);
          }
        } else {
  ...
  useEffect(() => {
    setCourse((prev) => {
      if (!prev) return prev;
      const totalDuration = prev.modules.reduce((acc, module) => {
        return (
          acc + module.lessons.reduce((lessonAcc, lesson) => lessonAcc + (lesson.duration || 0), 0)
        );
      }, 0);

      if (prev.duration === totalDuration) {
        return prev;
      }

      return { ...prev, duration: totalDuration };
    });
  }, [course?.modules]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (course.slug !== slug) {
          router.push(`/admin-premium/courses/${course.slug}/edit`);
        }
      } else {
  ...
  const addTag = () => {
    ...
    if (tagInput.trim() && !course.tags.includes(tagInput.trim())) {
      setCourse((prev) => (prev ? { ...prev, tags: [...prev.tags, tagInput.trim()] } : prev));
      setTagInput('');
    }
  };

  const removeTag = (tag: string) => {
    setCourse((prev) => (prev ? { ...prev, tags: prev.tags.filter((t) => t !== tag) } : prev));
  };
  ...
  const handleThumbnailUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCourse((prev) => (prev ? { ...prev, thumbnail: result.link } : prev));
  ...
  if (loading) {
    return (
      <div className="container flex max-w-5xl items-center justify-center py-20">
  ...
  return (
    <div className="container max-w-5xl py-8">
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              onCheckedChange={(checked) =>
                setCourse((prev) => (prev ? { ...prev, published: checked } : prev))
              }
            />
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={course.title}
                    onChange={(e) =>
                      setCourse((prev) => (prev ? { ...prev, title: e.target.value } : prev))
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={course.slug}
                    onChange={(e) =>
                      setCourse((prev) => (prev ? { ...prev, slug: e.target.value } : prev))
                    }
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  rows={4}
                  onChange={(e) =>
                    setCourse((prev) => (prev ? { ...prev, description: e.target.value } : prev))
                  }
                />
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value: 'beginner' | 'intermediate' | 'advanced') =>
                      setCourse((prev) => (prev ? { ...prev, difficulty: value } : prev))
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setCourse((prev) =>
                        prev ? { ...prev, xpReward: parseInt(e.target.value) || 0 } : prev
                      )
                    }
                  />
                </div>
      ...
      <LessonEditorDialog
        ...
        lesson={editingLesson ? editingLesson.lesson : null}
        onSave={saveLesson}
      />
    </div>
  );
