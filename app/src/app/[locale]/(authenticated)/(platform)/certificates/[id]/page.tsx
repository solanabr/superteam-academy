...
export default function CertificatePage() {
    ...
    const handleDownload = async () => {
        ...
        console.error = (...args: any[]) => {
            if (typeof args[0] === 'string' && (args[0].includes('unsupported color function "lab"') || args[0].includes('unsupported color function "oklab"'))) {
                return; // Ignore Next.js Turbopack / Tailwind v4 lab color css parse errors
            }
            originalConsoleError(...args);
        };

        try {
            // Wait for React to re-render with downloading=true
            ...
            const dataUrl = await toPng(certificateRef.current, {
                ...
                skipFonts: true, // Crucial: Fixes the 'font is undefined' and cssRules CORS errors
            });

            // Create a landscape PDF matching the aspect ratio of the captured canvas
            const img = new Image();
            img.src = dataUrl;
            ...
            // Add extra space at the bottom for the clickable link if valid
            const totalHeight = isValidExplorerLink ? imgHeightMm + 25 : imgHeightMm;
            ...
            // Fill background
            pdf.setFillColor(0, 0, 0); // Pure black for the link section
            pdf.rect(0, 0, pdfWidth, totalHeight, "F");

            pdf.addImage(dataUrl, "PNG", 0, 0, pdfWidth, imgHeightMm);

            // Directly embed the Solana Explorer link as an interactive PDF text block
            if (isValidExplorerLink) {
                const explorerUrl = `https://explorer.solana.com/address/${explorerAddress}?cluster=devnet`;
                // Set text color to Solana green (#14F195)
                pdf.setTextColor(20, 241, 149);
                pdf.setFontSize(14);

                const text = "Verify Official NFT Ownership on Solana Explorer";
                const textWidth = pdf.getTextWidth(text);
                const xPos = (pdfWidth - textWidth) / 2; // Center horizontally
                const yPos = imgHeightMm + 15; // Position below the image

                pdf.textWithLink(text, xPos, yPos, { url: explorerUrl });
            }

            pdf.save(`superteam-academy-certificate-${credential?.trackName?.toLowerCase() || 'award'}.pdf`);
        } catch (err) {
                        ...
                        <div ref={certificateRef} className="glass-panel rounded-2xl overflow-hidden border border-white/10 relative p-1 group">
                            ...
                            <div className="relative bg-void rounded-xl overflow-hidden border border-white/5 p-8 md:p-12 min-h-[500px] flex flex-col items-center justify-between bg-[url('/grid.svg')] bg-center backdrop-blur-sm">
                                ...
                                <div className="relative z-10 flex flex-col items-center text-center">
                                    ...
                                    <h2 className={`text-5xl font-display mb-6 drop-shadow-sm leading-tight uppercase font-bold tracking-tighter ${downloading ? 'text-white' : 'text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-400'}`}>
