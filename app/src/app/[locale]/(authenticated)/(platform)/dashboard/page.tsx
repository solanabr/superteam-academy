...
const CredentialList = dynamic(() => import("@/components/dashboard/CredentialList").then((mod) => mod.CredentialList), { ssr: false, loading: () => <div className="h-32 w-full animate-pulse bg-white/5 rounded-xl border border-white/10"></div> });
const ContinueLearning = dynamic(() => import("@/components/dashboard/ContinueLearning").then((mod) => mod.ContinueLearning), { ssr: false, loading: () => <div className="h-48 w-full animate-pulse bg-white/5 rounded-xl border border-white/10"></div> });
const StreakCalendar = dynamic(() => import("@/components/dashboard/StreakCalendar").then((mod) => mod.StreakCalendar), { ssr: false, loading: () => <div className="h-64 w-full animate-pulse bg-white/5 rounded-xl border border-white/10"></div> });
const ActivityFeed = dynamic(() => import("@/components/dashboard/ActivityFeed").then((mod) => mod.ActivityFeed), { ssr: false, loading: () => <div className="h-64 w-full animate-pulse bg-white/5 rounded-xl border border-white/10"></div> });

import { Button } from "@/components/ui/button";
import clsx from "clsx";
import { Wallet, AlertTriangle, ExternalLink, Activity, Flame, Medal, Check, Copy, Loader2, BookOpen, GraduationCap, Rocket, Sunrise, Moon, Puzzle, AlertCircle } from "lucide-react";
...
export default function DashboardPage() {
    ...
    // PRIORITY 1: Fetch progress (XP, Streak, Rank) immediately when wallet is available
    useEffect(() => {
        if (user?.walletAddress) {
            fetchProgress(user.walletAddress);

            // Fetch SOL balance in parallel (high priority for UX warnings)
    ...
    }, [user?.walletAddress, fetchProgress]);

    // PRIORITY 2: Fetch achievements and enrollments with a slight deferral
    useEffect(() => {
        if (user?.walletAddress) {
            const timer = setTimeout(() => {
                ...
                fetch("/api/courses/list")
                    ...
                    .catch(() => null);
            }, 100); // 100ms delay to give progress fetch the lead in the network queue

            return () => clearTimeout(timer);
        }
    }, [user?.walletAddress, fetchAchievements, fetchAllEnrollments]);
    ...
    if (!mounted) return <div className="min-h-screen bg-void flex items-center justify-center"><Loader2 className="animate-spin text-solana" /></div>;
    ...
    return (
        <main className="min-h-screen pt-4 pb-20">
            <div className="max-w-7xl mx-auto px-6 py-8 md:px-10 md:py-10 flex flex-col gap-10">
                ...
                {/* Streak Calendar & Activity Feed Row */}
                                        ...
                                        <div className={`hexagon-container relative w-20 h-24 transition-transform group-hover:scale-110 duration-300 ${trophy.claimed ? "drop-shadow-[0_0_10px_rgba(20,241,149,0.25)]" : ""}`}>
                                            <div className="hexagon-inner absolute inset-0 bg-gradient-to-br from-gray-800 to-black p-[1px]">
                                                ...
                                                <div className="absolute inset-[2px] bg-void hexagon flex items-center justify-center">
                                                    <AchievementIcon id={trophy.id} className={clsx("w-8 h-8", trophy.claimed ? "text-white" : "text-text-muted")} />
                                                </div>
                                            </div>
                                        </div>
                                        {trophy.claimed && (
