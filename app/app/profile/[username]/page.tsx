...
function getUnlockedAchievements(xp: number, level: number, enrolledCount: number, streak: number) {
  return {
    ...
    "early-adopter": true, // everyone who joined gets this
         ...
         <a     href={`https://explorer.solana.com/address/${username}?cluster=devnet`}
          ...
          <div className="flex items-start justify-between gap-8">
            <div className="flex items-center gap-6">
              {/* Avatar */}
              <div className="w-20 h-20 bg-gradient-to-br from-[#9945ff] to-[#14f195] flex items-center justify-center font-display font-black text-3xl text-white shrink-0">
                {displayName.slice(0, 2)}
              </div>
              <div>
                <h1 className="font-display font-black text-5xl uppercase tracking-tighter mb-2">
                  {displayName}
                </h1>
                <div className="flex items-center gap-4 text-[10px] font-mono uppercase tracking-widest">
                  ...
                  <span className="text-[#14f195] border border-[#14f195]/30 px-2 py-1">LEVEL {level}</span>
                  <span className="text-[#f5f5f0] border border-[#f5f5f0]/10 px-2 py-1">{enrolledCourses.length} COURSES</span>
                  <span className="text-[#ff3366] border border-[#ff3366]/30 px-2 py-1">🔥 {streak} DAY STREAK</span>
                </div>
              </div>
            </div>
            {isOwnProfile && (
      ...
      <div className="max-w-7xl mx-auto px-8 py-16 space-y-16">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            >
              <div className="text-[10px] font-mono text-[#444] uppercase tracking-widest mb-3">{stat.label}</div>
            ...
            {mockAchievements.map((achievement, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    "border p-5 flex flex-col items-center text-center transition-all",
                    isUnlocked
                      ? "border-[#9945ff]/40 bg-[#9945ff]/5 hover:bg-[#9945ff]/10"
                      : "border-[#1a1a1a] bg-[#0a0a0a] opacity-40"
                  )}
                >
                  <span className="text-4xl mb-3">{achievement.icon}</span>
                  <div className={cn(
                    "text-[10px] font-mono uppercase tracking-widest mb-2 font-bold",
                    isUnlocked ? "text-[#f5f5f0]" : "text-[#444]"
                  )}>
                    {achievement.name}
                  </div>
                  <div className="text-[9px] font-mono text-[#444] mb-3 leading-relaxed">
                  ...
                  {isUnlocked ? (
                    <div className="flex items-center gap-1 text-[9px] font-mono text-[#14f195] border border-[#14f195]/30 px-2 py-0.5">
                      <CheckCircle className="w-2.5 h-2.5" /> UNLOCKED
                    </div>
                  ) : (
                    <div className="flex items-center gap-1 text-[9px] font-mono text-[#333] border border-[#1a1a1a] px-2 py-0.5">
        ...
        <div>
          <div className="mb-8">
            ...
            <div className="font-display font-black text-3xl uppercase">Enrolled <span className="text-[#14f195]">Courses</span></div>
          </div>
                  ...
                  <Link href={`/courses/${course.id}`}>
                    <div className="flex items-center gap-6 px-8 py-5 hover:bg-[#0a0a0a] transition-colors group">
                      <div className="flex-1">
                        <div className="text-[10px] font-mono text-[#333] mb-1 uppercase">{course.track}</div>
        ...
        <div>
          <div className="mb-8">
            ...
            <div className="font-display font-black text-3xl uppercase">Soulbound <span className="text-[#ff3366]">NFTs</span></div>
          </div>
          <div className="border border-dashed border-[#1a1a1a] p-16 text-center">
